...
def show_score():
    ...
    label1 = font.render(f"Player Red:{player1_score}", True, RED)
    label2 = font.render(f"Player Yellow:{player2_score}", True, YELLOW)
...
def show_draw():
     # Εμφάνιση του ισοπαλίας
    font_path="game_font.ttf"
    font = pygame.font.Font(font_path, 100)
    text_surface = font.render(f"It's a draw!", True, BLUE)
    text_rect = text_surface.get_rect(center=(WIDTH // 2, HEIGHT / 15))
...
def play_game():
    ...
    game_over = False
    turn = 0
    draw_board(board)#δημιουργεί ταμπλό 
    pygame.display.update()#το εμφανίζει στην οθόνη
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if is_valid_location(board, col) :
                    ...
                    if winning_move(board, 1 if turn == 0 else 2):
                        ...
                        #προσθέτει τους πόντους
                        if winner==1:
                           player1_score += 1
                        elif winner==2:
                            player2_score += 1
                        print(f"Player {winner} wins!")
                        print(f"score for p1:{player1_score}-p2:{player2_score}")
...
def display_menu():
    ...
    reset_score=Button(screen, WIDTH // 4, HEIGHT // 5 * 3, WIDTH // 2, HEIGHT // 6, "Reset score")
    exit_button = Button(screen, WIDTH // 4, HEIGHT // 5 * 4, WIDTH // 2, HEIGHT // 6, "Exit")



    screen.fill(LIGHT_BLUE)#γεμίζει με ανοικτό μπλε την εικόνα
    show_score() # εμφανίζει το σκορ
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                #Αν πατηθεί το new_game κλείνει το παράθυρο του menu και αρχίζει νέο παιχνίδι
                 ...
                 #Αν πατηθεί το reset score επαναφερει το σκορ στο 0-0
                elif reset_score.is_clicked(mouse_pos):
                    player1_score=0
                    player2_score=0
                    show_score()
                    print(f"score for p1:{player1_score}-p2:{player2_score}")
...
# Κύριος κώδικας του παιχνιδιού
    ...
    reset_score=Button(screen, WIDTH // 4, HEIGHT // 5 * 2, WIDTH // 2, HEIGHT // 6, "Reset score")
